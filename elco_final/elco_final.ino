...
void setup_wifi() {
  ssid = "Redmi";      // SSID
  password = "elcoelco";      // Password
  host = "192.168.43.31";  // IP serveur - Server IP
  port = 3000;
}
...
void menu_button() {
  if (((millis() - lastDebounceTime) > debounceDelay) and menu_lock) {
    menu_lock = false;

    if (sleep_display) {
      display.ssd1306_command(SSD1306_DISPLAYON);
      sleep_display = false;
      current_state = 0; //fuerza a repintar la pantalla actual, para evitar bloqueo de menu_lock
      return;
    }
...
void mideTemp() {
  ...
  tempResult = (tempResult / TEMP_MUESTRAS) + 0.125 * (mlx.readAmbientTempC()); //offset en relación a una temperatura ambiente considerada estable




  Serial.print("\nTemperatura medida = "); Serial.print(tempResult);
  Serial.print("\nTemperatura ambiente = "); Serial.print(mlx.readAmbientTempC());
